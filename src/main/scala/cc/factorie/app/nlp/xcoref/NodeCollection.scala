...
import cc.factorie.variable.{BOWCubbie => BOWC, BagOfWordsVariable}
...
NC<:NodeCubbie[Vars, N]](val names:Seq[String], mongoDB:DB)(implicit ct: ClassTag[Vars]) extends DBNodeCollection[Vars, N, NC]{
  ...
  val varsCubbieColls = colls.tail.map(coll => new MongoCubbieCollection(coll,() => newBOWCubbie,(a:BOWC) => Seq(Seq(a.nodeId))) with LazyCubbieConverter[BOWC])
  ...
  def cubbifyBOW(nodeId:String, bow:BagOfWordsVariable) = bow.asHashMap.map{
    ...
    val nodes =
      for(nc <- nodeCubbieColl.toSeq) yield {
        ...
        val vars = varsCubbieColls.map(coll => {
          ...
          val bag = new BagOfWordsVariable(Seq())(null)
          for(b <- it){
  ...
  protected def newBOWCubbie : BOWC
